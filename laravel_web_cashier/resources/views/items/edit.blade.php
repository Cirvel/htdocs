        ...
        <div class="container-md form-control mx-auto p-2">
            <form class="" method="POST" action="{{ route('items.update',$item->id) }}">
                ...
                <div class="mb-3 d-flex">
                    <h3 class="form-label">EDIT ITEM</h3>
                    <div class="ms-auto">
                ...
                <div class="mb-3">
                    ...
                    title="Original value: {{ $item->name }}"
                    value="{{ $item->name }}"
                    required>
                </div>
                <div class="mb-3">
                    ...
                    title="Original value: {{ $item->description }}"
                    required>{{ $item->description }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="category_id" class="form-label">Category</label>
                    <select class="form-select" name="category_id"
                    title="Original value: {{ $item->categoryFK->name }}">
                        @foreach ($categories as $category )
                            <option title="{{ $category->description }}"
                                value="{{ $category->id }}"
                                {{ $item->categoryFK->name == $category->name ? "selected" : "" }}
                ...
                <div class="mb-3">
                    ...
                    title="Original value: {{ $item->quantity }}"
                    value="{{ $item->quantity }}"
                    required>
                </div>
                <div class="mb-3">
                    <label for="price" class="form-label">Price</label>
                    <input type="number" class="form-control" name="price" id="price" placeholder="Rupiah"
                    title="Original value: {{ $item->price }}"
                    value="{{ $item->price }}"
                    required>
                </div>
                <div>
